...
def load_and_preprocess_data(feature_columns, exclude_subject_id):
    # 데이터 로딩 및 전처리
    # 데이터 로딩
    ...
    if exclude_subject_id is not None:
        train_data = train_data[train_data["SubjectID"] != exclude_subject_id]
        val_data = val_data[val_data["SubjectID"] != exclude_subject_id]
        test_data = test_data[test_data["SubjectID"] != exclude_subject_id]
...
def train_model(model, criterion, optimizer, scheduler, train_loader, epoch, writer):
    ...
    for i, batch in enumerate(train_loader):
        ...
        x_batch = x_batch.unsqueeze(1)  # Adding a 1channel dimension for 1d cnn
        optimizer.zero_grad()
...
if __name__ == "__main__":
    ...
    (
        ...
        input_dim,
        output_dim,
    ) = load_and_preprocess_data(feature_columns, exclude_subject_id)
